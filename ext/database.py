...
def database_delete_message(message: discord.Message or classmethod): # Marks message as deleted in the message docs
    result = log_database[str(message.channel.id)].update_many({'message_id': str(message.id)}, {'$set': {'deleted': True}})
    if result is None:
        pass # error logger

async def database_get_last_message(bot: commands.Bot, guild_id: int, channel_id: int, message_id: int):
    message_doc = log_database[str(channel_id)].find_one({'message_id': str(message_id)}, sort=[('edited_at', DESCENDING)])
